...
/**
 * 动态稀疏线性层前向传播函数
 * 基于序列长度(seqlens)对每个样本动态应用稀疏矩阵乘法
 * 支持2D/3D输入张量，自动处理不同维度的输入
 * 
 * @param activation 输入激活值 [batch_size*seq_len, in_hidden] 或 [batch_size, seq_len, in_hidden]
 * @param weight 权重矩阵 [out_hidden, in_hidden]
 * @param bias 偏置向量 [out_hidden]
 * @param seqlens 每个样本的有效序列长度 [batch_size]
 * @return 输出张量 [batch_size*seq_len, out_hidden] 或 [batch_size, seq_len, out_hidden]
 */
...
)
{
    ...
    // 获取批次大小（seqlens的第一个维度）
    int batch_size = seqlens.size(0);
    
    // 计算最大序列长度，支持两种输入格式：
    // 3D: [batch_size, max_seq_len, hidden_dim]
    // 2D: [batch_size*max_seq_len, hidden_dim]
    int max_seq_len;
    if(activation.dim()==3){
        max_seq_len = activation.size(1);
    }
    ...
    // 获取输入/输出隐藏层维度，对应矩阵乘法的K和N
    int in_hidden = weight.size(1);  // 输入维度
    int out_hidden = weight.size(0); // 输出维度
    
    // 定义矩阵乘法维度：M=序列长度，K=输入维度，N=输出维度
    int M = max_seq_len;
    int K = in_hidden;
    int N = out_hidden;
    
    // 初始化输出张量，形状为[batch_size, max_seq_len, out_hidden]
    // 使用与输入相同的设备和数据类型
    torch::Tensor output = torch::zeros({batch_size, max_seq_len, out_hidden}, activation.options());
    
    // 根据输入数据类型(如float16/float32)自动调度执行
    // 使用AT_DISPATCH_FLOATING_TYPES_AND_HALF宏支持半精度和单精度浮点
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(activation.type(), "seqlen_dynamic_sparse_linear", ([&]
                            {       seqlen_dynamic_forward_function(
                                    activation.data_ptr<scalar_t>(),  // 输入激活值数据指针
                                    weight.data_ptr<scalar_t>(),      // 权重数据指针
                                    bias.data_ptr<scalar_t>(),        // 偏置数据指针
                                    seqlens.data_ptr<int>(),          // 序列长度数据指针
                                    M, K, N, batch_size,              // 矩阵维度和批次大小
                                    output.data_ptr<scalar_t>()       // 输出数据指针
                                ); }));
    
    // 根据输入维度调整输出形状，保持与输入一致的维度
    if(activation.dim()==3)
        return output;  // 直接返回3D张量
    else{
        return output.view({activation.size(0), out_hidden});  // 将3D展平为2D
    }
}
...
void backward_function(float * activation, float * weight, float * grad_in, int * seqlens, float*a_grad, float * w_grad, int M, int K, int N, int batch_size)
{
    // activation : M * K
    // weight: N * K
    // a_grad(MK) = MN*NK = grad_in * weight // NN sparse OUT
    // w_grad(NK) = NM * MK = grad_in^T * activation (sparse on K-dim), hard to optimize
    const int a_grad_M = M;
    const int a_grad_K = N;
    const int a_grad_N = K;
    dim3 a_grad_Grid(a_grad_N/32, a_grad_M/32, batch_size);
    dim3 a_grad_Block(256);
    BLOCK_SPARSE_MATMUL_OUT_NN_32_64_32<<<a_grad_Grid, a_grad_Block>>>(grad_in, weight, a_grad, seqlens, a_grad_M, a_grad_K, a_grad_N);
    ...
    torch::Tensor grad_c)
{
    ...
    if(activation.dim()==2){
        ...
        M = activation.size(0) * activation.size(1);
    }
    int batchsize = seqlens.size(0);
    assert(M%batchsize == 0);
    M = M /batchsize;
    torch::Tensor a_grad = torch::empty_like(activation);
    // printf("%d %d %d %d\n", batchsize, M, K, N);
    torch::Tensor w_grad = at::matmul(grad_c.view({M*batchsize, N}).t(), activation.view({M*batchsize,K})); // NEED to improve
    AT_DISPATCH_FLOATING_TYPES(activation.type(), "our_sparse_attention", ([&]
        { backward_function(
